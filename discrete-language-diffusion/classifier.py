...
class Classifier(BaseDMModel):
  ...
  def configure_optimizers(self):
    ...
    if self.train_time_independent:
      total_steps = self.config.trainer_ti.max_steps
    else:
      total_steps = self.config.trainer_td.max_steps
  ...
  def _compute_loss(self, batch, prefix):
    ...
    if self.is_eval_classifier:
      logits = self.forward(x0)
        ...
        time_cond, move_chance, _ = self._get_time_conditioning_and_move_chance(t)
