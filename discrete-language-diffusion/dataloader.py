...
def get_filename(config, subset_key, mode, tokenizer):
  dataset_name = config.data.train
  filename = dataset_name
  block_size = config.model.length
  wrap = config.data.wrap
  tok_tag = re.sub(r"[^a-zA-Z0-9]+", "-",  tokenizer.name_or_path.split("/")[-1])
...
def get_dataset(
  ...
  filename = get_filename(config, subset_key, mode, tokenizer)
  keys = [k for k in re.split(r"[,\s]+", subset_key) if k] if subset_key else ["whole_dataset"]
  ...
  if dataset_name == 'wikitext103':
    ...
    dataset = datasets.load_dataset(
      ...
      print(f"The dataset is of length: {len(dataset_filtered)}")
...
def get_tgt_src_domain_datasets(cfg, tokenizer):
    # Helpers
    def _balance(tgt_n, src_n):
        return "N/A" if src_n == 0 else f"{tgt_n / src_n:.2f}"

    log = lambda step, msg: print(f"[{step}] {msg}")

    # [LOAD] Build loaders
    train_ds_src, valid_ds_src = get_dataloaders(cfg, tokenizer, domain="src")
    train_ds_tgt, valid_ds_tgt = get_dataloaders(cfg, tokenizer, domain="tgt")

    src_n = len(train_ds_src.dataset)
    tgt_n = len(train_ds_tgt.dataset)
    log("LOAD TRAIN", f"src={src_n} tgt={tgt_n} balance={_balance(tgt_n, src_n)}")

    # [SPLIT] Move % of target into source
    pct_split = float(getattr(getattr(cfg, "loader"), "tgt_split_pct", 0.0))
    if 0 < pct_split < 100:
        idx_to_src = utils.compute_tgt_split_indices_rng(train_ds_tgt.dataset, pct_split, seed=cfg.loader.seed_for_splits)
        train_ds_src, train_ds_tgt = utils.apply_tgt_split_indices(train_ds_src, train_ds_tgt, idx_to_src)

        src_n = len(train_ds_src.dataset)
        tgt_n = len(train_ds_tgt.dataset)
        log("SPLIT", f"pct={pct_split:.1f}% src={src_n} tgt={tgt_n} balance={_balance(tgt_n, src_n)}")

    # [BALANCE] Compute drop to reach desired tgt/src
    desired = float(getattr(getattr(cfg, "loader"), "get_a_balance_between_src_tgt_samples", 1.0))
    current = (tgt_n / src_n) if src_n else 0.0
    keep_pct = 0.0 if current == 0 else max(0.0, min(100.0, 100.0 * (desired / current)))
    keep_pct_desired = float(getattr(getattr(cfg, "loader"), "lose_pct_tgt_samples", 1.0)) * 100.0
    keep_pct = min(keep_pct, keep_pct_desired)
    log("BALANCE", f"current={current:.2f} desired={desired:.2f} keep_pct={keep_pct:.1f}")

    # [DROP-TGT] Drop target samples if needed
    if 0 < keep_pct < 100:
        keep_idx = utils.compute_tgt_split_indices_rng(train_ds_tgt.dataset, keep_pct, seed=cfg.loader.seed_for_splits)
        train_ds_tgt = utils.drop_target_samples(train_ds_tgt, keep_idx)

        tgt_n = len(train_ds_tgt.dataset)
        log("DROP-TGT", f"kept_pct={keep_pct:.1f}% tgt={tgt_n} balance={_balance(tgt_n, src_n)}")

    return train_ds_src, valid_ds_src, train_ds_tgt, valid_ds_tgt