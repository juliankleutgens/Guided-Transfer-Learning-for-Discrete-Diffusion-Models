...
def _load_from_checkpoint(config, tokenizer, freeze=False):
    ...
    # case 2 ─ use checkpoint if it exists
        ...
        model, loaded = utils.build_or_load(
            ...
            freeze=freeze,          # or True if you want it frozen
        )
...
@L.pytorch.utilities.rank_zero_only
def _print_batch(train_ds=None, valid_ds=None, tokenizer=None, k=64):
  if tokenizer is None:
    raise ValueError("Tokenizer must be provided to print a batch.")
  if train_ds is None and valid_ds is None:
    raise ValueError("At least one of train_ds or valid_ds must be provided.")
...
def train_diffusion_model(cfg: DictConfig, callbacks: list, wandb_logger, tokenizer):
    ...
    train_ds_src, valid_ds_src, train_ds_tgt, valid_ds_tgt = dataloader.get_tgt_src_domain_datasets(cfg, tokenizer)
    #utils.print_i_beginning_and_ending_samples(train_ds_tgt, 3, tokenizer)
    domain_to_train_on = cfg.training.get("domain", "src")
    if domain_to_train_on == "src":
        train_ds = train_ds_src
        valid_ds = valid_ds_src
        print("Training on source domain.")
    elif domain_to_train_on == "tgt":
        train_ds = train_ds_tgt
        valid_ds = valid_ds_tgt
        print("Training on target domain.")
    else:
        raise ValueError(f"Unknown domain: {domain_to_train_on}")
    if cfg.eval.checkpoint_path:
        model = _load_from_checkpoint(cfg, tokenizer=valid_ds.tokenizer, freeze=False)
    else:
...
def generate_samples(config, logger, tokenizer):
  model = _load_from_checkpoint(config=config,tokenizer=tokenizer, freeze=True)
  ...
  for _ in range(config.sampling.num_sample_batches):
    ...
    elif config.mode == "sample_planner_eval" or config.mode == "full_evaluation":
      samples = model.restore_model_and_sample(use_planner_sampling=True)

    idxs_samples.extend(samples)
    text_sample = model.tokenizer.batch_decode(samples)
    print('Text samples:', text_sample)
  
  if config.sampling.guided_sampling:
    model.compute_topk_overlap_means()
  del model; torch.cuda.empty_cache()
  evaluate_samples.evaluate_the_generated_samples(idxs_samples, config, tokenizer)
...
def train_planner(cfg: DictConfig, callbacks: list, wandb_logger, tokenizer):
    ...
    # 1) Source loaders
    train_src, valid_src, _, _ = dataloader.get_tgt_src_domain_datasets(cfg, tokenizer)
    # 2) Frozen denoiser (labels provider)
    diff = _load_from_checkpoint(cfg, tokenizer)

    # 3) Build / load planner
    init_kwargs = dict(
        config=cfg,
        diffusion_model=diff,
        tokenizer=getattr(valid_src, "tokenizer", tokenizer),
    )
    planner_model, loaded = utils.build_or_load(
        model_=planner.Planner,
        init_kwargs=init_kwargs,
        ckpt_path=cfg.planner.ckpt_path,
        freeze=not cfg.planner.retrain_when_loaded,
    )

    # 4) Train if needed
    if (not loaded) or cfg.planner.retrain_when_loaded:
        wandb_planner = set_up_wandb_logger_for_clf_or_ratio_model(cfg, model_type="planner")
        val_metric = cfg.training_planner.get("val_metric_for_best_model", "val/cross_entropy")
        trainer = _make_trainer(
            cfg.trainer_planner,
            cfg.get("strategy"),
            utils._callbacks_for("planner", cfg.checkpointing.save_dir, callbacks, val_metric),
            wandb_planner,
        )
        trainer.fit(planner_model, train_src, valid_src, ckpt_path=cfg.planner.ckpt_path or None)

        # tidy-up
        wandb_planner.experiment.finish()
        planner_model.eval().cpu()
        trainer.strategy.teardown()
        del trainer
        torch.cuda.empty_cache()
    else:
        print(f"Planner loaded from {cfg.planner.ckpt_path}.")
...
def change_config_if_debugging(config):
  ...
  if config.gamma.get('guidance_scale', 0) == 0 and config.sampling.guided_sampling:
    print("Warning: guidance_scale is 0, disabling guided_sampling.")
    config.sampling.guided_sampling = False
